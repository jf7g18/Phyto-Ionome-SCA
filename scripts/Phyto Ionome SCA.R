...
#  │   ├── Calibration/
#  ├── Output/             
#  ├── Phytoplankton Ionome Analysis.R      
#  ├── Manual Cell Data.csv
#  ├── Model Cell Data.csv
#  ├── README.md
...
plot_boxplot <- function(element, data, tukey_groups, y_limits, y_breaks) {
  ...
  color_mapping <- setNames(c("grey", "red", "blue"), segmentation_levels)
...
std_dir <- paste0(base_dir, "LA-TOF-ICP-MS Data/Calibration") 
...
png(paste0(base_dir,"LA-TOF-ICP-MS Data/Calibration/NIST_heatmaps.png"), width = 900, height = 400, res = 150)
...
png(file.path(paste0(base_dir,"LA-TOF-ICP-MS Data/Calibration/NIST_Histogram_Panel.png")), width = 2500, height = 1000)
...
concs <- read.csv(paste0(base_dir,"LA-TOF-ICP-MS Data/Calibration/NIST Concentrations.csv"))[,-1]
...
ggsave(filename = paste0(base_dir, "LA-TOF-ICP-MS Data/Calibration/Calibration_Plots.png"), plot = panel_plot, width = 16, height = 10)

# Save regression results (Factors and R^2)
write.csv(results, file.path(paste0(base_dir, "LA-TOF-ICP-MS Data/Calibration/NIST Calibration Results.csv")), row.names = FALSE)
...
sample_dir <- paste0(base_dir, "LA-TOF-ICP-MS Data") 
...
factors <- read.csv(paste0(base_dir, "LA-TOF-ICP-MS Data/Calibration/NIST Calibration Results.csv"))[,2]
...
sample_list <- lapply(file.path(sample_dir, samples), function(dir) {
...
validation_dir <- paste0(base_dir, "Model Validation") 
mask_dir <- paste0(base_dir, "Masks")
...
image_dir <- paste0(base_dir, "Optical Images")
...
png(paste0(base_dir, "Output/S1_Optical_Model_Validation.png"), width = 4000, height = 2000, res = 500)
im <- jpeg::readJPEG(paste0(base_dir,"Optical Images/S1_validation.jpeg"))
x_cor <- 190:462
y_cor <- 0:272
...
png(paste0(base_dir, "Output/S2_Element_Model_Validation.png"), width = 4000, height = 2000, res = 500)
im <- jpeg::readJPEG(paste0(base_dir,"Optical Images/S2_cropped.jpg"))
x_cor <- 170:270
y_cor <- 0:100
layout(matrix(c(1, 2, 3, 4, 5, 6, 7, 8), ncol = 4, byrow = TRUE), widths = c(1, 1, 1, 1)) 
par(mar = c(1, 1, 1, 1))
...
for (layer in 1:length(mask_layers)) {
  ...
  png(filename = paste0(base_dir, "Output/", layer_names[layer], ".png"),
...
png(filename = paste0(base_dir, "Output/Aligned Sample Optical Images.png"))
layout(matrix(1:6, ncol = 1))
...
for (s in 1:4) {
  ...
  png(filename = paste0(base_dir, "Output/Cell_Element_Maps_", s, ".png"),
      width = base_width, height = base_height, res = 150)
  ...
  element_names <- c("Na", "Mn", "Mg", "Fe", "Si", "Cu", "P", "Zn")
  for (i in seq_along(element_indices)) {
    ...
    mtext(element_names[i], side = 3, line = -2, col = "white", adj = 0.1, cex = 1)
  }
  dev.off()
}
...
manual_cell_info <- read.csv(paste0(base_dir, "Manual Cell Data.csv"), header = TRUE)
combined$Manual$Species <- manual_cell_info[, 3]
combined$Manual$Volume <- manual_cell_info[, 4]

model_cell_info <- read.csv(paste0(base_dir, "Model Cell Data.csv"), header = TRUE)
...
write.csv(ionomes_data, file.path(paste0(base_dir, "Output/Total Cell Ionomes.csv")), row.names = FALSE)
...
cell_info <- read.csv(paste0(base_dir, "Model Cell Data.csv"), header = TRUE)
...
ggsave(paste0(base_dir, "Output/Cell Volumes and Community Composition.png"), plot = final_plot, width = 10, height = 6, dpi = 300)
...
ggsave(paste0(base_dir, "Output/Cell Ionome Boxplots.png"), plot = final_plot, width = 10, height = 6, dpi = 300)
...
ggsave(paste0(base_dir, "Output/PCA Ionome plots Station Species Volume.png"), plot = final_plot, width = 15, height = 6, dpi = 300)
